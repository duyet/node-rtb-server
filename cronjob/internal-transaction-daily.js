	...
	if (model && model.models && PublisherAgent && PublisherAgent.data) {
		...
		PublisherAgent.data.forEach(function(publisher) {
			...
			publisher.Website.forEach(function(Website) {
				var WebsiteObject = {
					PublisherInfoID 	: publisher.PublisherInfoID,
		...
		websiteStat.forEach(function(Website) {
			...
			.then(function(model) {
				if (model) {
					...
					model.save().then(function() {
						console.log("INFO: Update InternalTransaction in MySQL Server");	
					});
				} else {
			...
			// PublisherInfo
			new PublisherInfo({
				PublisherInfoID: Website.PublisherInfoID
			}).fetch()
			.then(function(model) {
				if (model) {
					var bl = model.get('Balance') || 0;
					var totalIncome = model.get('TotalIncome') || 0;
					model.set('MonthlyIncome', 	Website.NetMoney);
					model.set('Balance', 		bl + Website.NetMoney);
					model.set('TotalIncome', 	totalIncome + Website.NetMoney);
					model.save().then(function() {
						console.log("INFO: Update PublisherInfo ["+ Website.PublisherInfoID +"] in MySQL Server");	
					});	
				}
				else {
					console.log(Website.PublisherInfoID);
				}
			});
		});
	}
